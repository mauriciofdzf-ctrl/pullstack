...
export default function ChatBot() {
  ...
  return (
    <>
      ...
      {/* Panel de chat */}
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] max-h-[560px] flex flex-col bg-[#12161f] border border-white/10 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden">
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-3 bg-[#0d1018] border-b border-white/5">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
            {messages.map((m) => (
              <div key={m.id} className={`flex gap-2 ${m.from === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex flex-col gap-2 max-w-[85%] ${m.from === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`px-3 py-2.5 rounded-2xl text-sm leading-relaxed ${m.from === 'user' ? 'bg-violet-600 text-white font-medium rounded-br-sm' : 'bg-[#191d28] border border-white/5 text-gray-200 rounded-bl-sm'}`}>
                    {renderText(m.text)}
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {m.chips.map((c) => (
                        <button
                          ...
                          className="bg-[#191d28] hover:bg-violet-500/10 border border-white/10 hover:border-violet-500/40 text-gray-400 hover:text-violet-400 text-[11px] px-2.5 py-1 rounded-full transition-all"
                        >
                          {c}
                        </button>
                      ))}
            ...
            {typing && (
              <div className="flex gap-2">
                ...
                <div className="bg-[#191d28] border border-white/5 rounded-2xl rounded-bl-sm px-4 py-3 flex gap-1 items-center">
                  {[0, 1, 2].map((i) => (
          ...
          {/* Input */}
          <div className="px-3 py-3 border-t border-white/5 bg-[#0d1018]">
            <div className="flex gap-2 items-center">
              <input
                ...
                className="flex-1 bg-[#12161f] border border-white/10 text-white placeholder-gray-700 px-3 py-2.5 rounded-xl text-xs focus:outline-none focus:border-violet-500/40"
              />
              <button
