...
export default function Profile() {
  ...
  return (
    <div className="min-h-screen bg-[#06060f] pt-20 pb-16 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">

        {/* Header card */}
        <div className="bg-[#0e0e1e] border border-white/5 rounded-2xl overflow-hidden mb-6">
          {/* Banner */}
              ...
              <div className="flex gap-2 mt-4">
                {!editing ? (
                  <button onClick={() => setEditing(true)}
                    className="flex items-center gap-2 bg-[#161628] border border-white/10 hover:border-violet-500/30 text-gray-300 hover:text-violet-400 px-4 py-2 rounded-lg text-sm font-bold transition-all">
                    ✏️ Editar perfil
                  </button>
                ) : (
                  <>
            ...
            {editing ? (
              <div className="space-y-3">
                <input value={displayName} onChange={(e) => setDisplayName(e.target.value)}
                  ...
                  className="w-full bg-[#161628] border border-white/10 text-white px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:border-violet-500/50" />
                <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3}
                  placeholder="Cuéntanos sobre tu colección..."
                  className="w-full bg-[#161628] border border-white/10 text-white px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:border-violet-500/50 resize-none" />
              </div>
            ) : (
              <div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-6 bg-[#0e0e1e] border border-white/5 rounded-xl p-1">
        ...
        {/* Activity */}
        {activeTab === 'activity' && (
          <div className="bg-[#0e0e1e] border border-white/5 rounded-2xl divide-y divide-white/5">
            {recentActivity.length === 0 ? (
              ...
              recentActivity.map((a, i) => (
                <div key={i} className="flex items-center gap-4 p-4">
                  <div className="w-9 h-9 rounded-xl bg-[#161628] flex items-center justify-center text-lg shrink-0">{a.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm">{a.desc}</p>
                    <p className="text-gray-600 text-xs mt-0.5">{a.time}</p>
                  </div>
                  {a.amount && <span className="text-violet-400 font-black text-sm shrink-0">{a.amount}</span>}
                </div>
              ))
            )}
          </div>
        ...
        {activeTab === 'collection' && (
          <div className="bg-[#0e0e1e] border border-white/5 rounded-2xl p-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-3">
              ...
              <button onClick={() => navigate('/marketplace')} className="bg-[#161628] border border-white/10 hover:border-violet-500/30 text-gray-300 font-bold px-6 py-2.5 rounded-xl transition-all text-sm">
                Explorar cartas
              </button>
            </div>
          </div>
        ...
        {avatarEdit && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center px-4">
            <div className="bg-[#0e0e1e] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full bg-[#161628] border border-white/10 text-white placeholder-gray-600 px-4 py-3 rounded-xl text-sm focus:outline-none focus:border-violet-500/50 font-mono"
                />
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl text-sm font-black bg-violet-600 hover:bg-violet-500 text-white disabled:opacity-60 transition-all flex items-center justify-center gap-1.5"
                >
        ...
        {activeTab === 'settings' && (
          <div className="space-y-4">
            <div className="bg-[#0e0e1e] border border-white/5 rounded-2xl p-6 space-y-4">
              ...
              <div className="flex items-center justify-between py-3 border-b border-white/5">
                ...
                <span className="text-xs text-gray-600 bg-[#161628] px-2 py-1 rounded-lg">Verificado</span>
              </div>
              <div className="flex items-center justify-between py-3">
                ...
                <button className="text-violet-400 hover:text-violet-400 text-xs font-bold transition-colors">Cambiar</button>
              </div>
            </div>
            ...
            <div className="bg-[#0e0e1e] border border-red-500/10 rounded-2xl p-6">
