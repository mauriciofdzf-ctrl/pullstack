  ...
  return (
    <div className="min-h-screen bg-[#111128] pt-24 pb-16 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {user ? (
            ...
            <button onClick={() => navigate('/login')}
              className="bg-[#1a1a36] border border-white/10 hover:border-violet-500/30 text-gray-300 font-bold px-4 py-2.5 rounded-xl text-sm transition-all">
              Iniciar sesión
            </button>
          )}
        </div>
        ...
        {/* New post form */}
        {showForm && user && (
          <div className="bg-[#1a1a36] border border-violet-500/20 rounded-2xl p-5 mb-6 animate-in fade-in duration-200">
            <h3 className="text-white font-bold mb-4">Nueva publicación</h3>
            <input value={title} onChange={e => setTitle(e.target.value)}
              placeholder="Título *"
              className="w-full bg-[#21213e] border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm mb-3 focus:outline-none focus:border-violet-500/50 transition-colors" />
            <textarea value={body} onChange={e => setBody(e.target.value)}
              placeholder="Descripción, preguntas, pulls del día... (opcional)"
              rows={3}
              className="w-full bg-[#21213e] border border-white/10 text-white rounded-xl px-4 py-3 text-sm mb-3 focus:outline-none focus:border-violet-500/50 resize-none transition-colors" />
            <div className="flex items-center gap-3 flex-wrap">
              <select value={sport} onChange={e => setSport(e.target.value)}
                className="bg-[#21213e] border border-white/10 text-white rounded-xl px-3 py-2 text-sm focus:outline-none">
        ...
        {!user && (
          <div className="bg-[#1a1a36] border border-white/5 rounded-2xl p-5 mb-6 flex items-center justify-between gap-4">
            ...
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap border transition-all ${
                filter === s
                  ...
                  : 'bg-[#1a1a36] border-white/5 text-gray-400 hover:text-white hover:border-white/10'
              }`}>
              {s}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="bg-[#1a1a36] border border-white/5 rounded-2xl p-12 text-center">
          ...
          <div className="space-y-4">
            {posts.map(p => (
              <div key={p.id} className="bg-[#1a1a36] border border-white/5 hover:border-white/10 rounded-2xl p-5 transition-all">
