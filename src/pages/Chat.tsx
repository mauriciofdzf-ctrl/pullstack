              ...
              msgs.map((m, i) => {
                ...
                return (
                  <div key={m.id} className={`flex items-start gap-2.5 ${isMe ? 'flex-row-reverse' : ''}`}>
                    {!prevSame && (
                      <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getChatAvatarColor(m.user_id)} flex items-center justify-center text-white font-black text-[10px] shrink-0 mt-0.5`}>
                    ...
                    <div className={`max-w-xs lg:max-w-md ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                      {!prevSame && (
                        <div className={`flex items-baseline gap-2 mb-1 ${isMe ? 'flex-row-reverse' : ''}`}>
                          ...
                          <span className="text-gray-600 text-[10px]">{formatChatTime(m.created_at)}</span>
                        </div>
                      )}
