  ...
  if (success) return (
    <div className="min-h-screen bg-[#0a0a0a] pt-24 pb-16 px-4 flex items-center justify-center">
      <div className="max-w-md w-full text-center">
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 my-6 text-left space-y-2 text-sm">
          <p className="text-amber-400 font-bold">¿Qué sigue?</p>
        ...
        <p className="text-gray-500 text-xs mb-8">Costo de envío internacional aprox. <span className="text-amber-400 font-bold">$45 USD</span> round-trip desde México.</p>
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setSuccess(false); setForm({ player_name: '', card_year: '', brand: '', card_number: '', variation: '', raw_value: '', condition: 'Near Mint (NM)', grader: 'PSA', service_tier: 'Bulk', notes: '' }) }}
            className="bg-[#1a1a1a] border border-white/10 text-gray-300 font-bold px-5 py-2.5 rounded-xl text-sm transition-all hover:border-white/20">
            Nueva solicitud
          </button>
          <button onClick={() => navigate('/profile')}
            className="bg-amber-500 hover:bg-amber-400 text-black font-black px-5 py-2.5 rounded-xl text-sm transition-all">
            Ver mi perfil
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-bold px-3 py-1.5 rounded-full mb-4">
        ...
        <div className="grid grid-cols-4 gap-2 mb-8">
          {STEPS.map(s => (
            <div key={s.n} className="bg-[#111] border border-white/5 rounded-xl p-3 text-center">
              <div className="text-2xl mb-1.5">{s.icon}</div>
              <div className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/30 text-amber-400 font-black text-[10px] flex items-center justify-center mx-auto mb-1">{s.n}</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input value={form.player_name} onChange={e => set('player_name', e.target.value)}
                  ...
                  className="w-full bg-[#1a1a1a] border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-amber-500/50 transition-colors" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CONDITIONS.map(c => (
                    <button key={c} type="button" onClick={() => set('condition', c)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${form.condition === c ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' : 'bg-[#1a1a1a] border-white/10 text-gray-400 hover:border-white/20'}`}>
                      {c}
                    </button>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
              {GRADERS.map(g => (
                <button key={g.id} type="button"
                  ...
                  className={`relative p-3 rounded-xl border text-left transition-all ${form.grader === g.id ? 'bg-amber-500/10 border-amber-500/40' : 'bg-[#1a1a1a] border-white/10 hover:border-white/20'}`}>
                  {g.popular && (
                    <div className="absolute -top-2 -right-2 bg-amber-500 text-black text-[9px] font-black px-1.5 py-0.5 rounded-full">Popular</div>
                  )}
                  <div className={`font-black text-sm mb-0.5 ${form.grader === g.id ? 'text-amber-400' : 'text-white'}`}>{g.name}</div>
              ...
              <div className="space-y-2">
                {selectedServices.map(s => (
                  <button key={s.name} type="button" onClick={() => set('service_tier', s.name)}
                    className={`w-full flex items-center justify-between p-3 rounded-xl border text-left transition-all ${form.service_tier === s.name ? 'bg-amber-500/10 border-amber-500/40' : 'bg-[#1a1a1a] border-white/10 hover:border-white/20'}`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full border-2 shrink-0 ${form.service_tier === s.name ? 'bg-amber-500 border-amber-500' : 'border-gray-600'}`} />
                      <span className={`text-sm font-bold ${form.service_tier === s.name ? 'text-amber-400' : 'text-white'}`}>{s.name}</span>
                    </div>
          ...
          <div className="bg-[#111] border border-white/5 rounded-2xl p-6">
            ...
            <textarea value={form.notes} onChange={e => set('notes', e.target.value)}
              ...
              rows={3}
              className="w-full bg-[#1a1a1a] border border-white/10 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-500/50 resize-none transition-colors" />
          </div>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-black font-black py-4 rounded-2xl transition-all text-lg">
            {loading ? (
