      ...
      <div className="max-w-md w-full text-center">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setSuccess(false); setForm({ player_name: '', card_year: '', brand: '', card_number: '', variation: '', raw_value: '', condition: 'Near Mint (NM)', grader: 'PSA', service_tier: 'Bulk', notes: '' }) }}
            className="bg-[#191d28] border border-white/10 text-gray-300 font-bold px-5 py-2.5 rounded-xl text-sm transition-all hover:border-white/20">
            Nueva solicitud
          </button>
    ...
    <div className="min-h-screen bg-[#090c14] pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-2 mb-8">
          {STEPS.map(s => (
            <div key={s.n} className="bg-[#12161f] border border-white/5 rounded-xl p-3 text-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input value={form.player_name} onChange={e => set('player_name', e.target.value)}
                  ...
                  className="w-full bg-[#191d28] border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/50 transition-colors" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CONDITIONS.map(c => (
                    <button key={c} type="button" onClick={() => set('condition', c)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${form.condition === c ? 'bg-violet-500/20 border-violet-500/40 text-violet-400' : 'bg-[#191d28] border-white/10 text-gray-400 hover:border-white/20'}`}>
                      {c}
                    </button>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
              {GRADERS.map(g => (
                <button key={g.id} type="button"
                  ...
                  className={`relative p-3 rounded-xl border text-left transition-all ${form.grader === g.id ? 'bg-violet-500/10 border-violet-500/40' : 'bg-[#191d28] border-white/10 hover:border-white/20'}`}>
                  {g.popular && (
              ...
              <div className="space-y-2">
                {selectedServices.map(s => (
                  <button key={s.name} type="button" onClick={() => set('service_tier', s.name)}
                    className={`w-full flex items-center justify-between p-3 rounded-xl border text-left transition-all ${form.service_tier === s.name ? 'bg-violet-500/10 border-violet-500/40' : 'bg-[#191d28] border-white/10 hover:border-white/20'}`}>
          ...
          <div className="bg-[#12161f] border border-white/5 rounded-2xl p-6">
            ...
            <textarea value={form.notes} onChange={e => set('notes', e.target.value)}
              ...
              rows={3}
              className="w-full bg-[#191d28] border border-white/10 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-violet-500/50 resize-none transition-colors" />
          </div>
