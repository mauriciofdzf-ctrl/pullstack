...
function ImageSlot({ imgKey }: { imgKey: ImageKey }) {
  ...
  return (
    <div className={`bg-[#0d0d0d] rounded-2xl overflow-hidden border transition-all ${custom ? 'border-amber-500/40' : 'border-white/5'}`}>
      {/* Preview */}
      ...
      <div className="p-3 space-y-2">
        <input
          ...
          className="w-full bg-[#111] border border-white/10 text-white placeholder-gray-700 px-3 py-2 rounded-lg text-xs focus:outline-none focus:border-amber-500/50 font-mono"
        />
        <div className="flex gap-2">
          <button
            onClick={handleSave}
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${saved ? 'bg-green-500 text-white' : 'bg-amber-500 hover:bg-amber-400 text-black'}`}
          >
    ...
    <div className="min-h-screen bg-[#0a0a0a] pt-20 pb-16 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            <p className="text-amber-500 text-xs font-bold uppercase tracking-widest mb-2">PullStack · Creator</p>
        ...
        <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 mb-8 flex items-start gap-3">
          ...
          <div className="text-sm text-gray-400 leading-relaxed">
            ...
            <span className="text-gray-600 text-xs mt-1 block">
              Para producción: sube tus fotos a <a href="https://cloudinary.com" target="_blank" className="text-amber-400 underline">Cloudinary</a> (gratis) y usa esa URL. Evita URLs de Google Drive o WhatsApp — no funcionan en producción.
            </span>
          </div>
        </div>
          ...
          {IMAGE_SECTIONS.map((s) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all relative ${activeSection === s.label ? 'bg-amber-500 text-black' : 'bg-[#111] border border-white/10 text-gray-400 hover:border-amber-500/30 hover:text-amber-400'}`}
              >
                {s.label}
                {customCount > 0 && (
                  <span className={`ml-2 text-[10px] font-black ${activeSection === s.label ? 'text-black/60' : 'text-amber-500'}`}>
                    {customCount}✓
                  </span>
                )}
              </button>
        ...
        {/* Dónde se usan */}
        <div className="mt-10 bg-[#111] border border-white/5 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            {[
              ...
              <div key={i} className="flex gap-3">
                <span className="text-amber-400 font-bold shrink-0 text-xs pt-0.5 w-24">{row.slot}</span>
        ...
        {/* Tips de URLs buenas */}
        <div className="mt-6 bg-[#111] border border-white/5 rounded-2xl p-6">
