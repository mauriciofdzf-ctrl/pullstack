  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            {CONDITIONS.map((c) => (
              <button key={c.value} onClick={() => setCondition(c.value)}
                className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${condition === c.value ? 'border-violet-500/50 bg-violet-600/5 text-white' : 'border-white/10 bg-[#1d1d1d] text-gray-400 hover:border-violet-500/20'}`}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ v: 'modern', l: 'Moderna (2000+)' }, { v: 'recent', l: 'Reciente (2010+)' }, { v: 'vintage', l: 'Vintage (<1990)' }].map((e) => (
              <button key={e.v} onClick={() => setEra(e.v)}
                className={`py-2 px-2 rounded-lg border text-xs font-bold transition-all text-center ${era === e.v ? 'border-violet-500/50 bg-violet-600/5 text-violet-400' : 'border-white/10 bg-[#1d1d1d] text-gray-500 hover:border-violet-500/20'}`}>
                {e.l}
              </button>
            ))}
          </div>
      ...
      <div className="space-y-4">
        {!result ? (
          <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-6 flex flex-col items-center justify-center h-full gap-4 text-center">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-1 overflow-x-auto pb-2 mb-8 scrollbar-none">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all shrink-0 ${tab === t.id ? 'bg-violet-600 text-white' : 'bg-[#1a1a1a] border border-white/5 text-gray-400 hover:text-white hover:border-violet-500/20'}`}>
        ...
        {tab === 'tipos' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {CARD_TYPES.map((ct) => (
              <div key={ct.en} className="bg-[#1a1a1a] border border-white/5 hover:border-violet-500/20 rounded-2xl p-5 transition-all hover:-translate-y-0.5">
            ...
            {/* Parallels chart */}
            <div className="bg-[#1a1a1a] border border-violet-500/20 rounded-2xl p-5 sm:col-span-2 lg:col-span-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={e.label} className="bg-[#1a1a1a] border border-white/5 rounded-xl p-4">
        ...
        {tab === 'calc' && (
          <div>
            ...
            <GradingCalc />
            <div className="mt-6 bg-[#1a1a1a] border border-white/5 rounded-2xl p-5">
        ...
        {tab === 'valor' && (
          <div className="space-y-6">
            <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-6">
              ...
              <div className="mt-4 bg-[#1d1d1d] rounded-xl p-3 font-mono text-violet-400 text-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {VALUE_FACTORS.map((f) => (
                <div key={f.factor} className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-5 hover:border-violet-500/20 transition-all">
            ...
            <div className="bg-[#1a1a1a] border border-violet-500/20 rounded-2xl p-6">
        ...
        {tab === 'latam' && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {LATAM_TIPS.map((tip) => (
                <div key={tip.title} className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-5 hover:border-violet-500/20 transition-all">
                  ...
                  {[
                    ...
                    <div key={f.id}>
                      ...
                      <input type="number" id={f.id} placeholder={f.placeholder}
                        ...
                        className="w-full bg-[#1d1d1d] border border-white/10 text-white placeholder-gray-600 px-3 py-2.5 rounded-xl text-sm focus:outline-none focus:border-violet-500/50" />
                    </div>
                  ))}
                </div>
                <div id="latam-result" className="bg-[#1d1d1d] border border-white/5 rounded-xl p-4 flex items-center justify-center">
