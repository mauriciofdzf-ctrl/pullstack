                      ...
                      return (
                        <div key={m.id} className={`flex ${isMe ? 'flex-row-reverse' : 'flex-row'} items-end gap-2`}>
                          ...
                          <div className={`max-w-xs lg:max-w-sm ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                            ...
                            {m.listing_title && (
                              <div className="bg-[#21213e] border border-violet-500/20 rounded-xl px-3 py-2 mb-1 text-[10px] text-violet-400 font-bold">
                                📋 {m.listing_title}
                                {m.bid_amount && <span className="ml-2 text-red-400">Puja: {m.bid_amount}</span>}
                              </div>
                            )}
                            <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? 'bg-violet-600 text-white rounded-br-md' : 'bg-[#21213e] border border-white/5 text-gray-200 rounded-bl-md'}`}>
                              {m.content}
                            </div>
                  ...
                  <div className="px-4 py-3 border-t border-white/5 shrink-0">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-[#21213e] border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/50"
                      />
              ...
              {supportMsgs.map(m => (
                <div key={m.id} className={`flex ${m.from_admin ? 'justify-start' : 'justify-end'}`}>
                  ...
                  <div className={`max-w-xs lg:max-w-sm px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                    m.from_admin ? 'bg-[#21213e] border border-white/5 text-gray-300 rounded-tl-md' : 'bg-violet-600 text-white font-medium rounded-tr-md'
                  }`}>
                    {m.content}
                  </div>
                </div>
